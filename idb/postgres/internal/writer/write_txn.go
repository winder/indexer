...
func yieldTransactions(ctx context.Context, block *bookkeeping.Block, start uint64, payset []transactions.SignedTxnInBlock, outCh chan []interface{}) error {
	intra := uint(start)
...
// protocol
func AddTransactions(block *bookkeeping.Block, start uint64, payset []transactions.SignedTxnInBlock, tx pgx.Tx) error {
